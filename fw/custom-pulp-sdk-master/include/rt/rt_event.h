 ...
 *
 * This initialize the scheduler and makes it usable. As soon as this function is executed, events can be enqueued to
 * the scheduler.
 *
 * \param sched     Pointer to the scheduler structure. This structure must be allocated by the caller and must be kept
 * allocated until the scheduler is not used anymore.
 */
void rt_event_sched_init(rt_event_sched_t *sched);

/** \brief Allocate new events.
 *
 * Allocates the specified number of events and put them in the free list. These events can then be reserved through
 * other functions and pushed to the scheduler. Note that this function can be called several times in order to add more
 * events to the scheduler.
 *
 * \param sched     The scheduler for which the events are intended to be used. If NULL the default scheduler for the
 * current thread is used. \param nb_events The number of events to allocate. \return 0 if successfull, 1 otherwise.
 */
...
static inline rt_event_sched_t *__rt_event_get_current_sched() { return rt_event_internal_sched(); }
...
static inline void __rt_event_set_pending(rt_event_t *event) { event->implem.pending = 1; }

static inline void __rt_event_set_keep(rt_event_t *event) { event->implem.keep = 1; }
...
static inline void rt_event_yield(rt_event_sched_t *sched) { rt_event_execute(NULL, 1); }

static inline rt_event_sched_t *rt_event_internal_sched() { return &__rt_sched; }
...
static inline rt_event_t *__rt_init_event(rt_event_t *event, rt_event_sched_t *sched, void (*callback)(void *),
                                          void *arg) {
...
static inline __attribute__((always_inline)) void __rt_enqueue_event_to_sched(rt_event_sched_t *sched,
                                                                              rt_event_t *event) {
  event->implem.next = NULL;
  if (sched->first == NULL) {
    sched->first = event;
...
static inline void __rt_task_init(pi_task_t *task) { task->done = 0; }

static inline void __rt_task_init_from_cluster(pi_task_t *task) { task->implem.keep = 1; }

static inline void __rt_event_handle_end_of_task(pi_task_t *task) { rt_event_enqueue(task); }
